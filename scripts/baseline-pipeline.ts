...
/**
 * Run the v1 pipeline over every mesh in test/corpus/ and record a
 * baseline: pipeline completion plus a naive count of overlapping
 * 2D face pairs.
 *
 ...
 * triangle-pair sweep exists only to produce the v1 failure corpus.
 */
...
const outputPath = join(here, "..", "docs", "baseline-v1-pipeline.md");
...
const md = [
  ...
  ...rows.map(formatRow),
  "",
  `**Summary:** ${clean.length} of ${completed.length} models that completed the pipeline produced an overlap-free net under v1's plain-DFS unfolding.`,
  "",
].join("\n");
...
console.log();
console.log(
  `${clean.length} of ${completed.length} models produce overlap-free nets under v1's plain DFS.`,
);
console.log(`\nWrote ${outputPath}`);
