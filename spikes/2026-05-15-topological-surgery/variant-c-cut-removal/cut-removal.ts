 ...
 *
 * Spike code: kept self-contained — no edits to src/core/. Some
 * `any`-typed glue is used where TypeScript's strict mode and the
 * spike's throwaway character collide; flagged inline.
 */
...
import type { Piece, RecutResult } from "../../../src/core/recut.js";
...
interface Rigid {
  /**
   * In-place transform of a FlatFace's positions (returns a new
   * FlatFace; original is left untouched).
   */
...
const buildTransform = (
  ...
  // Find shared-edge positions and apexes in both faces.
  ...
  // Rotation: align (pB1 - pB0) → (pA1 - pA0).
  ...
  // Negative of the rotation that takes u → v: we rotate u to v, so
  // R(u) = v. cos θ and sin θ as computed give R = [[cosT, -sinT], [sinT, cosT]]
  // such that R * u/|u| = v/|v|. Verify: R*(ux,uy) where R rotates by θ,
  // R(u) = (cosT*ux - sinT*uy, sinT*ux + cosT*uy) — and we want this to
  // equal v scaled. With our cosT, sinT definitions, this is correct.

  // Step 1: translate so pB0 → origin.
  // Step 2: rotate by R.
  // Step 3: translate so origin → pA0.
  ...
  // Determine reflection: apex of A vs transformed apex of B
  // should be on opposite sides of line (pA0, pA1).
  const tApexB = apply1(pBApex);
  const sideA = signed(pA0, pA1, pAApex);
  const sideB = signed(pA0, pA1, tApexB);
  // If same sign (same side), reflect across the shared-edge line.
  ...
  if (!needReflect) {
    ...
    // Reflect across the line through pA0 with direction (pA1 - pA0).
    ...
    apply2 = (p: Vec2): Vec2 => {
      ...
      const qx = r[0] - pA0![0];
      const qy = r[1] - pA0![1];
      // parallel component along (ex, ey)
      const pp = qx * ex + qy * ey;
      // reflect: q_parallel - q_perp, where q_perp = q - q_parallel*e
      const ppx = pp * ex;
      const ppy = pp * ey;
      const perpX = qx - ppx;
      const perpY = qy - ppy;
...
export function runCutRemoval(
  ...
  const uf = makeUF(faceCount);

  // Sort adjacencies by 3D edge length descending — long cuts are
  // the most expensive to keep, so we try to fold them back first.
  ...
  for (const idx of order) {
    ...
    // Identify which component is the "anchor" (don't move) and
    // which is "moved." Anchor by larger size, tie-break by
    // smaller root index — keeps movement modest.
    ...
    // Determine which mesh face is in anchor's set and which is in mover's.
    ...
    // Commit: apply transform to mover's positions permanently.
